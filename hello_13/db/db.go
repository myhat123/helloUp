...
//金融下载平台连接字符串
func getCPDSN(prefetch_rows int) string {
	s1 := "test/test@cpdata"
	s2 := prefetch_rows
	s := fmt.Sprintf("%s?prefetch_rows=%d", s1, s2)
	return s
}

//共享平台连接字符串
func getGXDSN(prefetch_rows int) string {
	s1 := "petl/test@ycldb"
	s2 := prefetch_rows
	s := fmt.Sprintf("%s?prefetch_rows=%d", s1, s2)
	return s
...
func getDbTable(plat string, table string, date ...string) (d *sql.DB, t string) {
	...
	if plat == "cp" {
		db, _ = sql.Open("oci8", getCPDSN(20000))
		table_name = getCPTableName(table, date...)
	} else if plat == "gx" {
		db, _ = sql.Open("oci8", getGXDSN(20000))
